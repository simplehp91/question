...
<?php
			include('config.php');
			$counts=0;
			$rscount = mysql_query("SELECT * FROM tbl_hocsinh");
  ...
  <form action="?menu=hocsinh_delete" method="post">
    <table cellpadding="1" cellspacing="1" id="resultTable">
		...
		<?php
			...
			$results = mysql_query("SELECT * FROM tbl_hocsinh ");
			while($rows = mysql_fetch_array($results))
			{
			
			$dates=explode("-",$rows['ngaysinh']);
				$y=$dates[0];
				$m=$dates[1];
				$d=$dates[2];
				$strdate=$d."/".$m."/".$y;
			...
			<td>'.$rows['diachi'].'</td>
			<td>'.$rows['lop'].'</td>
			<td><a href="?menu=hocsinh_edit&idhs='.$rows['idhs'].'"><img src="images/go_right.gif" /></a></td>
			</tr>';
			}
		?>
        </tbody>
